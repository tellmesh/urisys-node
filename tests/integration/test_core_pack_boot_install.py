...
from urisysnode.serve import _register_pack  # noqa: E402
...
def test_core_pack_auto_install_on_boot(tmp_path):
    ...
    with patch("urisysnode.serve.auto_install_enabled", return_value=True):
        with patch("urisysnode.serve.ensure_boot_pack", return_value={"ok": True}) as pip:
            with patch("urisysnode.serve.importlib.import_module", side_effect=_screen_import_once_then_ok):
...
def test_core_pack_boot_raises_when_auto_install_disabled(tmp_path):
    rt = Runtime(events_path=str(tmp_path / "events.jsonl"))
    with patch("urisysnode.serve.auto_install_enabled", return_value=False):
        with patch(
            "urisysnode.serve.importlib.import_module",
...
def test_core_pack_boot_raises_when_pip_fails(tmp_path):
    ...
    with patch("urisysnode.serve.auto_install_enabled", return_value=True):
        with patch(
            "urisysnode.serve.ensure_boot_pack",
